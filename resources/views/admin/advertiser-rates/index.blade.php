...
@section('title', 'Advertiser Click Rates')
...
<div style="display:grid;grid-template-columns:1fr 360px;gap:24px;align-items:start;" class="rates-layout">
    ...
    <div class="card">
            ...
            <table>
                ...
                <tbody id="ratesTableBody">
                    ...
                    <tr class="rate-row" data-name="{{ strtolower($rate->country_name) }}" data-code="{{ strtolower($rate->country_code) }}">
                        ...
                        <td>
                            <form method="POST" action="{{ route('admin.advertiser-rates.update', $rate) }}" style="display:flex;gap:6px;align-items:center;">
                                @csrf @method('PUT')
                        ...
                        <td>
                            <form method="POST" action="{{ route('admin.advertiser-rates.update', $rate) }}" style="display:inline;">
                                @csrf @method('PUT')
                        ...
                        <td>
                            <form method="POST" action="{{ route('admin.advertiser-rates.destroy', $rate) }}" onsubmit="return confirm('Delete this rate?')">
                                @csrf @method('DELETE')
        ...
        @else
        <div style="text-align:center;padding:40px;color:var(--text-muted);">No rates configured yet. Add a country rate on the right.</div>
        @endif
    </div>
    ...
    <div>
        <div class="card" style="position:sticky;top:80px;">
            <div style="font-size:15px;font-weight:700;margin-bottom:20px;">Add Country Rate</div>
            <form method="POST" action="{{ route('admin.advertiser-rates.store') }}">
                @csrf
                ...
                <div class="form-group">
                    <label class="form-label">Rate per Click (USD) <span style="color:#ef4444;">*</span></label>
                    <div style="display:flex;align-items:center;gap:6px;">
                        <span style="font-size:14px;color:#6b7280;font-weight:600;">$</span>
                        <input type="number" name="rate_usd" class="form-control" value="{{ old('rate_usd') }}"
                               step="0.000001" min="0.000001" placeholder="0.050000" required>
                    </div>
                </div>
        ...
        <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:14px 16px;margin-top:16px;font-size:13px;color:#1e40af;line-height:1.6;">
            <strong>How rates work:</strong><br>
            When you set a <em>per-country</em> contract for an advertiser, clicking <strong>"Load Default Rates"</strong> in the campaign form auto-fills these rates. You can still adjust per-campaign before sending the contract.
        </div>
    </div>
