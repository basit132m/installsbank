...
@section('title', 'Country Click Rates')
...
<div style="background:linear-gradient(135deg,#0f172a 0%,#1e1b4b 50%,#0f172a 100%);border-radius:20px;padding:24px 32px;margin-bottom:24px;position:relative;overflow:hidden;">
    ...
    <div style="display:flex;gap:24px;align-items:center;flex-wrap:wrap;position:relative;">
        <div style="flex:1;min-width:180px;">
            <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#64748b;margin-bottom:6px;">Per-Click Revenue</div>
            <div style="font-size:24px;font-weight:900;color:#f8fafc;letter-spacing:-0.5px;line-height:1.1;">Country Click Rates</div>
            <div style="font-size:13px;color:#64748b;margin-top:4px;">Set how much publishers earn per valid Windows click by country</div>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;flex-shrink:0;">
            <div style="background:rgba(1,191,99,.12);border:1px solid rgba(1,191,99,.25);border-radius:14px;padding:12px 20px;text-align:center;min-width:100px;">
                ...
                <div style="font-size:24px;font-weight:900;color:#01BF63;line-height:1;">{{ $rates->total() }}</div>
...
<div id="bulkBar" style="display:none;position:sticky;top:64px;z-index:100;background:linear-gradient(135deg,#1e293b,#0f172a);color:#fff;padding:14px 22px;border-radius:14px;margin-bottom:18px;align-items:center;gap:14px;flex-wrap:wrap;box-shadow:0 8px 32px rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.08);">
    <div style="display:flex;align-items:center;gap:8px;">
        <div style="width:28px;height:28px;background:rgba(1,191,99,.2);border-radius:8px;display:flex;align-items:center;justify-content:center;">
            <svg width="14" height="14" fill="none" stroke="#01BF63" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
        </div>
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-left:auto;flex-wrap:wrap;">
        ...
        <button onclick="applyBulkRate()" type="button"
                style="padding:8px 18px;background:linear-gradient(135deg,#01BF63,#059669);color:#fff;border:none;border-radius:9px;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 3px 10px rgba(1,191,99,.4);">
            Apply to Selected
        </button>
...
<div style="background:#fffbeb;border:1.5px solid #fde68a;border-radius:14px;padding:16px 20px;margin-bottom:20px;display:flex;align-items:center;gap:16px;">
    ...
    <div>
        <div style="font-size:14px;font-weight:800;color:#92400e;">{{ $unratedCount }} {{ Str::plural('country', $unratedCount) }} detected with no rate set</div>
        <div style="font-size:13px;color:#b45309;margin-top:3px;">Auto-detected from incoming clicks. Clicks are tracked but earnings show <strong>N/A</strong> until you set a rate. They are highlighted in amber below.</div>
    </div>
</div>
@endif
...
<div style="display:grid;grid-template-columns:300px 1fr;gap:20px;align-items:start;">
    ...
    <div style="position:sticky;top:80px;">
        <div class="card" style="border:1.5px solid #e5e7eb;">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
                <div style="width:40px;height:40px;background:linear-gradient(135deg,#01BF63,#059669);border-radius:11px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 4px 12px rgba(1,191,99,.25);">
                ...
                <div>
                    <div style="font-size:14px;font-weight:800;color:#111827;">Add Country Rate</div>
                    <div style="font-size:12px;color:#9ca3af;margin-top:1px;">Set per-click earnings</div>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('admin.rates.store') }}">
                ...
                <div class="form-group" style="margin-bottom:20px;">
                    ...
                    <div style="position:relative;">
                        ...
                        <input type="number" name="rate_per_click" class="form-control" placeholder="0.000500" step="0.000001" min="0" required style="padding-left:26px;font-family:monospace;">
                    </div>
                </div>
                <button type="submit" class="btn btn-primary" style="width:100%;background:linear-gradient(135deg,#01BF63,#059669);border:none;box-shadow:0 4px 12px rgba(1,191,99,.3);">
                    Add Country Rate
                </button>
            </form>
    ...
    <div class="card" style="padding:0;overflow:hidden;border:1.5px solid #e5e7eb;">
        ...
        <div style="padding:18px 20px;border-bottom:1px solid #f3f4f6;display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
            ...
            <div>
                <div style="font-size:14px;font-weight:800;color:#111827;">All Country Rates</div>
            ...
            <div style="margin-left:auto;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                ...
                <button type="submit" form="bulkForm"
                        style="display:flex;align-items:center;gap:7px;padding:9px 18px;background:linear-gradient(135deg,#01BF63,#059669);color:white;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 4px 12px rgba(1,191,99,.3);">
                ...
                <table style="width:100%;border-collapse:collapse;">
                    <thead>
                        <tr style="background:#f9fafb;border-bottom:1px solid #e5e7eb;">
                            ...
                            <th style="text-align:left;padding:10px 14px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;color:#6b7280;">Rate / Click</th>
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom:1px solid #f3f4f6;{{ $rate->needs_rate_update ? 'background:#fffef0;' : '' }}"
                            ...
                            <td style="padding:12px 14px;">
                                <div style="display:flex;align-items:center;gap:6px;">
                                    ...
                                    <input type="number"
                                           name="rates[{{ $rate->id }}][rate_per_click]"
                            ...
                            <td style="padding:12px 14px;text-align:center;">
                                <button type="button"
                                        onclick="if(confirm('Delete rate for {{ addslashes($rate->country_name) }}?')) document.getElementById('del-cr-{{ $rate->id }}').submit();"
                        ...
                        <tr>
                            <td colspan="6" style="text-align:center;padding:48px 24px;color:#9ca3af;">
                                ...
                                <div style="font-size:14px;font-weight:600;">No rates configured yet</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
...
document.querySelectorAll('.rate-input').forEach(input => {
    ...
    input.addEventListener('focus', function() {
        this.style.borderColor = '#01BF63';
        this.style.boxShadow   = '0 0 0 3px rgba(1,191,99,.15)';
    });
