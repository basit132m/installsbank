...
// favicon.ico — always blank; installsbank.com gets its icon via inline SVG in the layout
Route::get('/favicon.ico', function () {
    return response('', 204)->header('Cache-Control', 'public, max-age=86400');
});
...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'role:admin,manager'])->group(function () {
    ...
    Route::prefix('traffic-reports')->name('traffic-reports.')->middleware('permission:can_generate_reports')->group(function () {
        ...
        Route::get('/{trafficReport}', [Admin\TrafficReportController::class, 'show'])->name('show');
        Route::delete('/{trafficReport}', [Admin\TrafficReportController::class, 'destroy'])->name('destroy');
    });
